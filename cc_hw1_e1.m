...
load('517_BER_mex_win.mat');
BER_mex_win = n_err./(pck_sent*pck_len);
prec = 1 - log10(pck_len*it_num);
disp(['Expected precision = 10^' num2str(prec)])
...
plot(ebn0_dB(1:end-1), log10(BER_mex(1:end-3)), '^-'), hold on
%plot(ebn0_dB(1:end-1), log10(BER_matlab(1:end-2)), 'v-'), hold on
plot(ebn0_dB(1:end-1), log10(BER_mex_win(1:end-1)), 'x-'), hold on

legend('g3', 'uncoded', 'nominal coding gain', 'viterbi mex', 'viterbi windowed')
