	...
	std::vector<double> *noise_vector) 
{
	...
	std::mt19937 rng(rd());
	std::uniform_real_distribution<double> sigma_noise(0, sigma_w/8);
	...
	// decode, and add some noise to sigma_w estimate

	std::chrono::time_point<std::chrono::system_clock> begin = std::chrono::system_clock::now();
	std::vector<bool> *decoded_symbols = decoder->decode(received_signal, std::pow(sigma_w + sigma_noise(rng), 2));
...
int main(int argc, char const *argv[])
{
		...
		if((attempt+1)%100 == 0) {
			for(int snr_write = 0; snr_write < num_SNR; snr_write++) {
				if(num_attempt_per_snr[snr_write] >= attempt + 1) {
					...
					conc_filename << "simulation_results_multi_sw_" << ebn0_vec[snr_write] << ".txt";
	...
	std::ofstream output_file ("BER_and_time_multi_sw.txt", std::ios::out | std::ios::app);
    for (int snr_ind = 0; snr_ind < num_SNR; ++snr_ind)
    {
